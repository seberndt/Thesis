...
(real-valued) \myDef{random variable}, which is a mapping from $\Omega$
...
\myDef{expected value} of $X$ as $\Exp[X] := \sum_{x\in X(\Omega)}x\cdot
\Pr[X=x]$. Two random variables $X$ and $Y$ are \myDef{independent
  random variables}, if $X^{-1}(x)$ and $Y^{-1}(y)$ are independent
...
by the machine. Similarly, the \myDef{running time} of a \ac{PTM} is defined
...
bounded by a polynomial, we say that $\machine$ is a \myDef{\ac{PPTM}} or an
efficient algorithm.
...
Similarly, a function $f\colon U\to V$ is \myDef{efficiently
  computable}, if there is a \ac{PPTM} $\machine$ such that
...
A \myDef{hash function} $(\sfh,\Gen_{\sfh})$ is a pair of \acp{PPTM} such that
...
\myDef{collision finder} $\Fi$ for a hash function $(\sfh,\Gen_{\sfh})$
...
experiment:
\myAlgorithm{$\Coll_{\Fi,(\sfh,\Gen_{\sfh})}(\kappa)$}{Hash function $(\sfh,\Gen_{\sfh})$, Finder $\Fi$, key length $\kappa$}{
\State $k \gets \Gen_{\sfh}(1^{\kappa})$
\State $(x,x') \gets \Fi(k)$
\If{$x\neq x'$ and $\sfh_{k}(x)=\sfh_{k}(x')$} 
   \AlgReturn{1}
\Else
 \AlgReturn{0}
\EndIf
...
from completely random function. A \myDef{\ac{PRF}} $(\sff,\Gen_{\sff})$ is a
...
$\ell'(\kappa)$. An attacker, called \myDef{distinguisher} $\Dist$ upon
input $1^{\kappa}$ gets
...
say that the pair $(\sff,\Gen_{\sff})$ is a \myDef{\ac{PRP}}.
...
$k\in \{0,1\}^{*}$ with $|k|\geq \kappa$. The \myDef{encryption
...
\myDef{random counter mode}. Let $(\sff,\Gen_{\sff})$ be a \ac{PRF} that maps
...
\myAlgorithm{$\Enc^{\sff}$}{key $k$, $m=m_{1}m_{2}\ldots m_{n(\kappa)}\in
  \{0,1\}^{n(\kappa)\cdot \ell(\kappa)}$}{
\State $\kappa := |k|$
\State $r \sgets \{0,1\}^{\ell(\kappa)}$ \Comment{$r$ is treated as
  string \emph{and} number}
\For{$i=1,\ldots,n(\kappa)$}
\State $c_{i} := \sff_{k}(r+i \bmod 2^{\ell(\kappa)})\oplus m_{i}$
\EndFor
...
\myAlgorithm{$\Dec^{\sff}$}{key $k$, $c=c_{0}c_{1}\ldots c_{n(\kappa)}\in
  \{0,1\}^{(n+1)\cdot \ell(\kappa)}$}{
\State $\kappa := |k|$
\State $r := c_{0}$
\For{$i=1,\ldots,n(\kappa)$}
\State $m_{i} := \sff_{k}(r+i \bmod 2^{\ell(\kappa)})\oplus c_{i}$
\EndFor
\State \AlgReturn $m_{1},m_{2},\ldots,m_{n(\kappa)}$
}{Random Counter Mode Decryption}
\end{itemize}

Clearly, every ciphertext $c=\Enc^{\sff}(k,m)$ is decoded correctly. Concerning
...
\begin{theorem}[Theorem 13 in the full version of \cite{bellare1997concrete}]
